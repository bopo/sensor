...
" > supervisor.conf
sudo cp supervisor.conf /etc/supervisor/conf.d/sensor.conf
rm supervisor.conf
...
sudo cp nginx.conf /etc/nginx/sites-available/sensor
sudo ln -s /etc/nginx/sites-available/sensor /etc/nginx/sites-enabled/sensor
...
sudo su postgres sh -c "createuser sensor -P -d"
sudo su postgres sh -c "createdb sensor"
